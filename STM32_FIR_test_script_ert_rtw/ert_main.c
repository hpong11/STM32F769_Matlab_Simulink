...
int main(void)
{
  ...
  rtmSetErrorStatus(STM32_FIR_test_script_M, 0);
  STM32_FIR_test_script_initialize();
  ARMCM_SysTick_Config(modelBaseRate);
  runModel =
    rtmGetErrorStatus(STM32_FIR_test_script_M) == (NULL);
  __enable_irq();
  __enable_irq();
  while (runModel) {
    stopRequested = !(
  ...
  /* Terminate model */
  STM32_FIR_test_script_terminate();
